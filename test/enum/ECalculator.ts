...
export class ECalculator extends EnumType<ECalculator>() {
    ...
    private constructor(readonly operator: string, readonly calculator: (x: number, y: number) => number) {
        super();
    }

    calc(operator: string, x: number, y: number) {
        return ECalculator.values().filter(e => e.operator === operator).pop().calculator(x, y);
    }
}